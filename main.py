...
def get_mask_expanded(mask, shift_amount):
    ...
    Parameters:
    mask (numpy.ndarray): A 2D binary numpy array where 1 represents text pixels
                            and 0 represents image pixels.
    ...
    shifts = [
        (offset_x, offset_y)
        for offset_x in (0, shift_amount, -shift_amount)
    ...
    for offset in shifts:
        ...
        mask = np.clip(
            mask_shifted + mask, 0, 1
        )  # ensures combined mask values remain within [0,1]
...
def enhance_image(
    image_path, enhanced_dir, color=1.0, contrast=1.0, sharpness=1.0, brightness=1.0
):
    """
    ...
    Parameters:
        ...
        color/contrast/sharpness/brightness (float):
            Factor by which to enhance the respective. Default to unchanged (1.0).
...
def get_translated_ocr(image_ocr_path):
    """
    Usually text in memes are split into multiple lines, better concatenate
    to full sentence for correct translation.
    If one word a line, might be addressing a person rather than a sentence.
    Parameters:
        image_ocr_path (str): Path to the ocr text and coordinates json.
        Dictionary with text as key and coordinates as values.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--mode', type=str, required=True, choices=['clean', 'enhance', 'translate']
    )

    parser.add_argument(
        '--img_dir',
        type=str,
        help='Relative path to raw image folder',
        required=False,
        default='img/',
    )
    parser.add_argument(
        '--cleaned_dir',
        type=str,
        help='Relative path to directory text-removed images',
        required=False,
        default='img_cleaned/',
    )
    parser.add_argument(
        '--enhanced_dir',
        type=str,
        help='Relative path to enhanced image folder',
        required=False,
        default='img_enhanced/',
    )
    parser.add_argument(
        '--ocr_dir',
        type=str,
        help='Relative path to folder storing ocr text and coordinates',
        required=False,
        default='ocr/',
    )
    parser.add_argument(
        '--translated_dir',
        type=str,
        help='Relative path to translated image folder',
        required=False,
        default='img_translated/',
    )
